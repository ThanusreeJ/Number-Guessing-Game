    ...
    try:
        if i==0:
            ...
            if m<0 or n<0:
                ...
                taking_range()
            low,high = checkrange(m,n) #function call and swapping if possible
            num=random.randrange(low,high)
            print(f"I'm thinking of a number between {low} and {high}.")
            print("Done with thinking :) ")
            game(score,player,i)#function call
        else:
...
def dictionary(player,score,i):
    global score_dict
    try: #{chaithu : (score, age[]) }
        score_dict[player] = [score , age_list[i]]
    except IndexError:
        pass
def check_winners():#(0,1)
    try: #{chaithu : (23,2),shashank: (23,5)}
    ...
    while(num_attempts<=max_attempts ):
        try:
                ...
                if guess==num:
                    ...
                    dictionary(player,score,i)# Function call
                    break
                #elif guess < low:
                 #   print("Your guessed number {} is lesser than the lowest boundry, try again!".format(guess))
               # elif guess > high:
                #    print("Your guessed number {} is greater than the highest boundry, try again!".format(guess))
...
def game_Starts():#Fuction declaration 
    players=get_players()        #calling get_players function to note the No.of players , names

        #loop for the players traversing
    for i in range(len(players)):
        taking_range(players[i],i)#function call
    check_winners()
    #Start of the game with set of rules        
...
print("Let's get started!\n")
    #Initializing global variables for furthur usage
num_attempts=0
max_attempts=10
num_hints=0
low=0
...
num=0
num_players=0
        #Creating a list for storing the age of players
age_list=list()

        #Creating a list for storing the names of players
players=list()

        #Create a dictionary to store the score of players corresponding to the players names
score_dict=dict()
        # storing value like this {chaitu : (30,45)}
game_Starts()
